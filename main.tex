...
In this work, the fundamental quantity needed in order to constrain the BH spin in M87* is the efficiency of jet production $\eta \equiv P/ \dot{M} c^2$, where $P$ is the power carried by the relativistic jet--the jet power--and $\dot{M}$ is the mass accretion rate onto the SMBH. These are the two M87* observables that we need in order to apply our models of jet production to constrain the spin parameter.
...
We use the results of global, general relativistic magnetohydrodynamic (GRMHD) simulations of radiatively inefficient accretion flows (RIAFs) around Kerr BHs to model the dependence of the jet power on the $a_*$ and $\Phi$. Concretely, we model the jet power as $P = \eta(a_*, \Phi) \dot{M} c^2$ where the jet production efficiency 
\begin{equation}
\eta = \left( \frac{\phi}{15} \right)^2 f(a_*)
\end{equation}
is a function of both the spin and dimensionless magnetic flux $\phi \equiv \Phi/(\dot{M} r_g^2)^{1/2}$. We use the GRMHD results of  \cite{Sasha2012a} who carried out RIAF simulations with $h/r \approx 0.3$ to set the spin-dependence of $\eta$, where $h$ is the disk thickness. We explore the astrophysically relevant range of magnetic fluxes, ranging from the ``magnetically arrested disk'' (MAD) case for which $\phi \sim 15$ \citep{Narayan2003, Sasha2011} to the ``standard and normal disk evolution'' (SANE) for which $\phi \sim 1$ \citep{Narayan2012}. Figure  \ref{spin-model} illustrates the spin-dependence of the $\eta$. Notice that this model encompasses both the prograde and retrograde cases in which the disk and BH are rotating in the same and opposite senses, respectively. Retrograde BHs produce less powerful jets \citep{Sasha2012}.
...
Our second result is a mapping of the observed efficiecies to spins, magnetic flux
explain that the model jet efficiency depends on spin and flux
hence the range of observed efficiencies allows us to set constraints on these values
\mario
